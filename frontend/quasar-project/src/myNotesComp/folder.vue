<template>
  <q-card
    ...
    v-on:click="this.store.selectedNotes.length ? addToSelecteds(data) : openFolder(data)"
    v-for="(data,key) in this.store.myNotesFolders" :key="key"
    class="q-ml-sm card col-12 col-md-4 col-sm-4 text-white" style="height:220px;"
    :class="this.checkSelectedNotes(data) ? 'bg-dark'  : ''"
    >
    <img
      ...
      width="1920"
      style="heigth:220px;object-fit:cover;"
    />
    <div class="card__content | flow">
      <div class="card__content--container | flow">
        <h2 class="card__title">{{data.folderName ?? 'No Folder Name'}}</h2>
...
<script>
import folerImage from '../images/folderImage.png'
...
export default {
  ...
  methods:{
    ...
    openFolder(data){
      ...
      console.log(allBody)
      this.$router.push(
        {path:`/noteFolder/${id}/${noteId}`,params:{allBody}}
      )
